...
shinyServer(function(session, input, output) {
    ...
    data <- callModule(dataInput, "loadnfilter",
    ...
    ## PHENOTABLE MODULE
    callModule(phenotypeTable, "phenotab", data$meData, phenoModRep, addPheno)
    ## FEATURE TABLE MODULE
    callModule(featureTable, "featuretab", data$meData, featureModRep)
    ...
    aggResults <- callModule(aggregationTab, "aggregateTab",
    ...
    intraRep <- callModule(intraAnalysis, "intraAnalysis", data, levelOpts,
                           chosenLevel, resetInput, aggData, normalizedData)

    ## INTER SAMPLE ANALYSIS
    interRep <- callModule(interAnalysis, "interAnalysis", data, levelOpts,
                           chosenLevel, resetInput, aggData)

    ## CORRELATION ANALYSIS
    corrRep <- callModule(corrAnalysis, "corrAnalysis", data, levelOpts,
                           chosenLevel, resetInput, aggData)

    ## DIFFERENTIAL ANALYSIS
    diffRep <- callModule(diffAnalysis, "diffAnalysis", data, levelOpts,
                           chosenLevel, resetInput, aggData, normalizedData)
    
    ## DIFFERENTIAL ANALYSIS
    longRep <- callModule(longAnalysis, "longAnalysis", data, levelOpts,
                          chosenLevel, resetInput, aggData, normalizedData)
    ...
    ## REPORT TAB
    callModule(reportList, "reportlist",
        dataSource = dataSource,
